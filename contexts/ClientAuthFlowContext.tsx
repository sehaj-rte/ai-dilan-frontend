...
interface ClientAuthFlowContextType {
  ...
  // Main Flow Handler
  handleChatOrCall: (publication: Publication, sessionType: 'chat' | 'call') => Promise<void>
...
const ClientAuthFlowContext = createContext<ClientAuthFlowContextType | undefined>(undefined)

export function ClientAuthFlowProvider({ children }: { children: React.ReactNode }) {
  ...
  const [currentSessionType, setCurrentSessionType] = useState<'chat' | 'call' | null>(null)
   ...
   */
  const handleChatOrCall = async (publication: Publication, sessionType: 'chat' | 'call') => {
  ...
  const handleLogin = async (email: string, password: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (loginData.success) {
        ...
        return { success: false, error: loginData.detail || 'Invalid email or password' }
      }
    } catch (error) {
   ...
   */
  const isExpertOwner = async (userId: string, expertId: string): Promise<boolean> => {
    try {
      const token = localStorage.getItem('dilan_ai_token')
   ...
   * Check if user has already paid for this expert
   */
  const hasUserPaid = async (userId: string, publicationId: string): Promise<boolean> => {
    try {
      const token = localStorage.getItem('dilan_ai_token')
      if (!token) return false
  ...
  return (
    <ClientAuthFlowContext.Provider value={{
      showAuthModal,
      setShowAuthModal,
      showPaymentModal,
      setShowPaymentModal,
...
export function useClientAuthFlow() {
  ...
  if (context === undefined) {
    throw new Error('useClientAuthFlow must be used within a ClientAuthFlowProvider')
  }
  return context
}
